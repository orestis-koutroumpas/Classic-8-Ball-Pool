...
Cuestick::Cuestick(glm::vec3 cueBallPos) {
    ...
    // Compute the default direction (facing the cue ball)
    vec3 forward = normalize(vec3(0.0f, 0.0f, 1.0f));  // Default forward direction

    // Offset the cue stick backwards from the cue ball
    float initialDistance = 0.75f;  // Default offset distance
...
void Cuestick::update(glm::vec3 cueBallPos, float horizontalAngle) {
    ...
    // Constrain spin effect inside ball radius (0.025f)
