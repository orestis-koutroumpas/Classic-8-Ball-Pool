...
void setupBallPositions(float ballRadius) {
    ballPositions.clear();
    ballPositions.reserve(16); // Avoid multiple memory allocations

    float xOffset = 3.25f * ballRadius;
    float zOffset = sqrt(3.75f) * ballRadius;
    float yOffset = 0.0f;
    ...
    for (int row = 0; row < 5; ++row) {
        for (int col = 0; col <= row; ++col) {
            float x = row * xOffset;
            float z = (col * 2 - row) * zOffset;
            ballPositions.push_back(vec3(x, yOffset, z));
            ballIndex++;
        }
    }

    // Set cue ball position
    ballPositions.push_back(vec3(-55.0f * ballRadius, yOffset, 0.0f));
}
...
// ---- Drawing functions fro lighting pass --- //
...
// Handles collisions between balls, table, holew adds friction
...
void mainLoop() {
    float lastTime = static_cast<float>(glfwGetTime());  // Store previous frame time
    do {
...
void pollKeyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        if (key == GLFW_KEY_Q && game.aimMode) {
            std::cout << "Q key pressed. Exiting aiming mode." << std::endl;
            camera->lock = false;
            game.aimMode = false;
        }
